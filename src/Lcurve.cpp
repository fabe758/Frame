...
template <class Type> void Lcurve<Type>::process_while(int in_size) {
  if (c_lcp.bench)
    c_lcp.start = std::clock();
  for (size_t is = 0; is < c_srcm.size(); is++) {
    if (is >= c_frtri.size())
  ...
  for (size_t is = 0; is < c_srcm.size(); is++)
    for (size_t it = 0; it < c_t.size(); it++)
      c_frtri[is][it].process_while(in_size);
  if (c_lcp.bench)
    c_lcp.stop = std::clock();
};

template <class Type> void Lcurve<Type>::process_while(Type ratio_unc_in) {
  if (c_lcp.bench)
    c_lcp.start = std::clock();
  for (size_t is = 0; is < c_srcm.size(); is++) {
    if (is >= c_frtri.size())
  ...
  for (size_t is = 0; is < c_srcm.size(); is++)
    for (size_t it = 0; it < c_t.size(); it++)
      c_frtri[is][it].process_while(ratio_unc_in);
  if (c_lcp.bench)
    c_lcp.stop = std::clock();
};

template <typename Type> void Lcurve<Type>::process_while2(int in_size) {
  if (c_lcp.bench)
    c_lcp.start = std::clock();
  c_mag2 = std::vector<std::vector<Type>>(c_srcm.size(),
                                          std::vector<Type>(c_t.size()));
  c_as2 = std::vector<V2d<Type>>(c_t.size());
  ...
  for (size_t is = 0; is < c_srcm.size(); is++) {
    ...
    for (size_t it = 0; it < c_t.size(); it++) {
      ...
      c_mag2[is][it] = frtri.area_sn() / frtri.src().area();
      br_tot[it] += frtri.area_sn();
      c_as2[it] += frtri.sump_area();
      if (c_lcp.record) {
  ...
  for (size_t it = 0; it < c_t.size(); it++)
    c_as2[it] = c_as2[it] / br_tot[it];
  if (c_lcp.bench)
    c_lcp.stop = std::clock();
}

template <typename Type> void Lcurve<Type>::process_while2(Type ratio_unc_in) {
  if (c_lcp.bench)
    c_lcp.start = std::clock();
  c_mag2 = std::vector<std::vector<Type>>(c_srcm.size(),
                                          std::vector<Type>(c_t.size()));
...
std::vector<std::vector<Type>> Lcurve<Type>::mags2() const {
  return c_mag2;
}

template <typename Type> std::vector<Type> Lcurve<Type>::mag2() const {
  return c_mag2[0];
}

template <typename Type> std::vector<V2d<Type>> Lcurve<Type>::as2() const {
  return c_as2;
}
